...
| Tool | Обязательные параметры | Примечания |
|------|------------------------|------------|
| `list_stickers` | — | Возвращает `[{id, name, states: [имяСостояния]}]`. Но ID состояний здесь НЕ выдаются. Чтобы получить ID состояний, открой задачу с уже выставленным состоянием и посмотри её поле `stickers` через `get_task` |
| `set_task_stickers` | `taskId`, `stickers` (объект) | Заменяет стикеры. Формат: `{"stickerId": "stateId"}` или `{"stickerId": true}` для булевых стикеров |

**Как узнать ID состояний (workaround):** в UI YouGile поставь нужное состояние
на любой задаче, потом вызови `get_task` и посмотри поле `stickers` — там
настоящие ID. Закешируй их.
...
4. `list_stickers` → найди ID стикера «Приоритет»
5. (Один раз) выясни ID состояний: создай или открой задачу в UI YouGile с
   нужным приоритетом, затем `get_task` и посмотри `stickers`. Закешируй ID.
6. `create_task` с `title`, `columnId`, `stickers: {приоритетId: highStateId}`
...
- **Имена состояний стикеров vs ID**: `list_stickers` показывает имена, но API
  хранит ID. См. секцию workaround выше.
